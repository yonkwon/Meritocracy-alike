...
public class MatWriter {
  ...
  MatWriter(Computation c) {
    Matrix matrixArrayM = Mat5.newMatrix(new int[]{1, Main.M_LENGTH});
    Matrix matrixArrayM0 = Mat5.newMatrix(new int[]{1, Main.M_LENGTH});
    Matrix matrixArrayM1 = Mat5.newMatrix(new int[]{1, Main.M_LENGTH});
    Matrix matrixArrayDeltaProbability = Mat5.newMatrix(new int[]{1, Main.P_LENGTH});
    Matrix matrixArrayDeltaUtility = Mat5.newMatrix(new int[]{1, Main.DELTA_U_LENGTH});

    IntStream.range(0, Main.M_LENGTH).forEach(i -> matrixArrayM.setDouble(new int[]{0, i}, Main.M[i]));
    IntStream.range(0, Main.M_LENGTH).forEach(i -> matrixArrayM0.setDouble(new int[]{0, i}, Main.M0[i]));
    IntStream.range(0, Main.M_LENGTH).forEach(i -> matrixArrayM1.setDouble(new int[]{0, i}, Main.M1[i]));
    IntStream.range(0, Main.P_LENGTH).forEach(i -> matrixArrayDeltaProbability.setDouble(new int[]{0, i}, Main.P[i]));
    IntStream.range(0, Main.DELTA_U_LENGTH).forEach(i -> matrixArrayDeltaUtility.setDouble(new int[]{0, i}, Main.DELTA_U[i]));

    Matrix matrixArrayComposition = Mat5.newMatrix(new int[]{Main.M_LENGTH, 2});
    for (int m = 0; m < Main.M_LENGTH; m++) {
      matrixArrayComposition.setDouble(new int[]{m, 0}, Main.COMPOSITION[m][0]);
      matrixArrayComposition.setDouble(new int[]{m, 1}, Main.COMPOSITION[m][1]);
    }

    Matrix matrixArrayG = Mat5.newMatrix(new int[]{Main.G_LENGTH, 2});
    for (int g = 0; g < Main.G_LENGTH; g++) {
      matrixArrayG.setDouble(new int[]{g, 0}, Main.G[g][0]);
      matrixArrayG.setDouble(new int[]{g, 1}, Main.G[g][1]);
    }

    Matrix consensusShareMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix consensusType0ShareMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix consensusType1ShareMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix perfectShareMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix perfectType0ShareMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix perfectType1ShareMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix mixedCoalitionShareMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);

    Matrix decisionAgainstPreferenceAVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_TYPE);
    Matrix decisionAgainstPreferenceSTDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_TYPE);
    Matrix choiceRateAVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_ARM);
    Matrix choiceRateSTDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_ARM);
    Matrix choiceRateIndAVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_IND_ARM);
    Matrix choiceRateIndSTDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_IND_ARM);

    Matrix falsePositiveCNTMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_IND_ARM);
    Matrix falsePositiveAVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_IND_ARM);
    Matrix falsePositiveSTDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_IND_ARM);
    Matrix falseNegativeCNTMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_IND_ARM);
    Matrix falseNegativeAVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_IND_ARM);
    Matrix falseNegativeSTDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE_IND_ARM);

    Matrix vote2Win00AVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix vote2Win00STDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix vote2Win01AVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix vote2Win01STDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix vote2Win10AVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix vote2Win10STDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix vote2Win11AVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix vote2Win11STDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);

    Matrix organizationalDecision0AVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix organizationalDecision0STDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix organizationalDecision1AVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix organizationalDecision1STDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix femaleVote0AVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix femaleVote0STDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix femaleVote1AVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix femaleVote1STDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);

    Matrix utilityAVGMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    Matrix utilitySTDMatrix = Mat5.newMatrix(Main.RESULT_KEY_VALUE);
    ...
    try {
      Mat5.newMatFile()
          .addArray("para_is_learning", Mat5.newScalar(Main.IS_LEARNING ? 1 : 0))
          .addArray("para_is_greedy", Mat5.newScalar(Main.IS_GREEDY ? 1 : 0))
          .addArray("para_is_prejoin_random", Mat5.newScalar(Main.IS_INITIAL_RANDOM ? 1 : 0))
          ...
          .addArray("para_g_m", Mat5.newScalar(Main.M_LENGTH))
          .addArray("para_a_comp", matrixArrayComposition)
          .addArray("para_a_dp", matrixArrayDeltaProbability)
          .addArray("para_g_dp", Mat5.newScalar(Main.P_LENGTH))
          .addArray("para_a_du", matrixArrayDeltaUtility)
          .addArray("para_g_du", Mat5.newScalar(Main.DELTA_U_LENGTH))
          .addArray("para_a_g", matrixArrayG)
          .addArray("para_g_g", Mat5.newScalar(Main.G_LENGTH))
          .addArray("para_tau", Mat5.newScalar(Main.TAU))
          .addArray("para_g_dr", Mat5.newScalar(Main.DECISION_STRUCTURE_LENGTH))
